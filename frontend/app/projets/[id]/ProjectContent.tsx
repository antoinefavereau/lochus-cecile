    ...
    <>
      <div className="w-full flex flex-col gap-4 md:gap-8 md:px-16">
          ...
          <div className="hidden md:flex align-center gap-16 text-light">
            {[project.year, project.category].map((info) => (
              <div key={info} className="relative pb-6">
                <SliceInText
                  animationRef={info === project.year ? dateRef : categoryRef}
                >
                  {info}
                </SliceInText>
                <span
                  ref={info === project.year ? dateLineRef : categoryLineRef}
        ...
        <Image
          ...
          height={1080}
        />
        <p className="md:hidden text-light self-end pe-10">{project.year}</p>
      </div>
      ...
      {!!project.without_background_image && (
      ...
      <div className="w-full flex flex-col gap-6 md:gap-8 px-8 md:px-16">
        ...
        >
          {project.paragraph_content.map((content, index) => (
            <p key={index} className="max-w-4xl text-base md:text-2xl fade-up">
              {content}
            </p>
          ))}
        </div>
